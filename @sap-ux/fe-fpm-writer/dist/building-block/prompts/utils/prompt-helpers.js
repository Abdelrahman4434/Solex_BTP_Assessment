...
exports.entitySetCache = void 0;
exports.loadEntitySets = loadEntitySets;
exports.getEntitySetOptions = getEntitySetOptions;
exports.resolveEntitySetTargets = resolveEntitySetTargets;
exports.resolveBindingContextTypeChoices = resolveBindingContextTypeChoices;
exports.getButtonGroupsChoices = getButtonGroupsChoices;
 ...
 */
async function loadEntitySets(context) {
    ...
    if (!exports.entitySetCache[cacheKey]) {
        exports.entitySetCache[cacheKey] = await (0, service_1.getEntitySets)(project, appId);
    }
    return exports.entitySetCache[cacheKey];
}
/**
 ...
 */
function getEntitySetOptions(entitySets, pageContextEntitySet, bindingContextType) {
    ...
    if (bindingContextType === types_1.bindingContextRelative) {
        let navigationProperties = contextEntitySet?.entityType?.navigationProperties?.filter((navProp) => navProp.isCollection === false) ??
            [];
        navigationProperties = navigationProperties.filter((navProp) => !['DraftAdministrativeData', 'SiblingEntity'].includes(navProp.name));
 ...
 */
function resolveEntitySetTargets(entitySets, pageContextEntitySet, bindingContextType, selectedContext) {
    ...
    if (bindingContextType === types_1.bindingContextRelative && selectedContext) {
 ...
 */
function getBindingContextTypeChoices(disableRelative = false) {
    const t = (0, i18n_1.translate)(i18n_1.i18nNamespaces.buildingBlock, 'prompts.');
    return [
        { name: t('common.bindingContextType.option.absolute'), value: types_1.bindingContextAbsolute },
        {
            name: t('common.bindingContextType.option.relative'),
            value: types_1.bindingContextRelative,
 ...
 */
function resolveBindingContextTypeChoices(context) {
    ...
    if (project) {
        return async () => {
            ...
            const options = getEntitySetOptions(entitySets, pageContextEntitySet, types_1.bindingContextRelative);
            if (!options.length) {
 ...
 */
async function getButtonGroupsChoices(context, answers) {
    const { project, appPath, fs } = context;
    if (!project) {
        return [];
    }
    // Get existing button groups from XML if viewOrFragmentPath is available
    let existingButtonGroups = new Set();
    if (answers.viewOrFragmentPath) {
        const xmlFilePath = (0, node_path_1.join)(appPath, answers.viewOrFragmentPath);
        existingButtonGroups = await (0, xml_1.getExistingButtonGroups)(xmlFilePath, answers.aggregationPath, fs);
    }
    ...
    return processor_1.BUTTON_GROUP_CONFIGS.map((config) => {
        const name = t(`choices.${config.name}`);
        return {
            name: name,
            value: config.name,
