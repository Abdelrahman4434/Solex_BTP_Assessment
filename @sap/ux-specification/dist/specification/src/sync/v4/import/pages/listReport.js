...
function transferColumns(config, importParameters, factory, pageType, routingId, configId, viewId) {
    ...
    if (lineItems && Object.keys(lineItems.properties).length > 0) {
        ...
        const viewsCount = views && typeof views === 'object' && views.paths?.length !== undefined ? views.paths.length : 0;
        if (viewsCount < 1) {
...
function createListReportConfig(importParameters, schemaType) {
    ...
    for (const key in v4Pages) {
        ...
        const matchesId = v4Page.name && key === importParameters.routingId;
        const matchesSchema = [ux_specification_types_1.PageTypeV4.ListReport, ux_specification_types_1.PageTypeV4.AnalyticalListPage].includes(pageType);
        if (matchesId && matchesSchema) {
