...
const types_1 = require("@sap/ux-specification-types/src/common/types");
/**
...
/**
 * Determines if the provided action definition is of a specific action type or matches specific criteria.
 *
 * @param actionDefinition The action definition object to evaluate, which may contain properties like actionType.
 * @param formActionDefinition The form action definition object to evaluate, which may contain annotationType information.
 * @returns A boolean indicating whether the given action or form action definition meets the specified conditions for being a recognized action type.
 */
function isSupportedAction(actionDefinition, formActionDefinition) {
    const isAction = formActionDefinition[types_1.SchemaTag.annotationType] === "com.sap.vocabularies.UI.v1.DataFieldForAction" /* UIAnnotationTypes.DataFieldForAction */;
    const isActionGroup = formActionDefinition[types_1.SchemaTag.annotationType] === "com.sap.vocabularies.UI.v1.DataFieldForActionGroup" /* UIAnnotationTypes.DataFieldForActionGroup */;
    const isCustomActionMenu = actionDefinition?.[types_1.SchemaTag.actionType] === ux_specification_types_1.ActionType.CustomMenu;
    const isCustomAction = formActionDefinition.description === 'Custom Action';
    return isAction || isActionGroup || isCustomAction || isCustomActionMenu;
}
/**
...
function fillFormActionsInConfig(importParameters, manifestSection, sectionId, configPart, factory, manifestSectionId) {
    if (manifestSection['actions']) {
        ...
        for (const actionKey in actionsDefinition['properties']) {
            ...
            if (isSupportedAction(actionDefinition, formActionDefinition)) {
                const importActionParams = {
...
/**
 * Fills custom fields in config.json.
 *
 * @param importParameters - object comprising all input data
 * @param manifestSection - section of the manifest.json containing the relevant settings
 * @param sectionId - ID of the current section
 ...
 * @param factory - factory, for generating the access to reflect-metadata
 * @param manifestSectionId - ID of the section in the manifest
 * @param sectionDefinitionName - definition key for fields section
 */
function fillFieldInConfig(importParameters, manifestSection, sectionId, configPart, factory, manifestSectionId, sectionDefinitionName = ux_specification_types_1.DefinitionName.ObjectPageSectionForm) {
    if (manifestSection['fields']) {
        const forms = importParameters.jsonSchema.definitions[`${sectionDefinitionName}<${getDefinitionId(manifestSectionId)}>`];
        const formReference = forms.properties.form['$ref'].split(common_1.DEFINITION_LINK_PREFIX)[1];
        const form = importParameters.jsonSchema.definitions[formReference];
        const fields = form.properties['fields'];
        const fieldsReference = fields.$ref.split(common_1.DEFINITION_LINK_PREFIX)[1];
        const fieldsDefinition = importParameters.jsonSchema.definitions[fieldsReference];
        for (const fieldKey in fieldsDefinition['properties']) {
            const fieldDefinition = fieldsDefinition.properties[fieldKey];
            const definitionKey = (0, common_1.getDefinitionKey)(fieldDefinition.$ref);
            const formFieldDefinition = importParameters.jsonSchema.definitions[definitionKey];
            if (!('annotationPath' in formFieldDefinition) && formFieldDefinition.description === 'Custom Field') {
                configPart[sectionId].form ??= {};
                configPart[sectionId].form.fields ??= {};
                const schemaPropertyName = definitionKey.split('<')[0];
                const fieldInstance = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, schemaPropertyName);
                configPart[sectionId].form.fields[fieldKey] = fieldInstance;
                (0, utils_1.importSettingsOfObject)(fieldInstance, importParameters.manifest, formFieldDefinition, importParameters.routingId, [fieldKey], sectionId, undefined, importParameters.jsonSchema.definitions);
            }
        }
    }
}
...
/**
 * Traverses the keys and values of a TypeScript `enum` (or enum-like object),
 * invoking a callback for each entry. Entry whose value matches `lastEntry` will be visited as last.
 *
 * @param enumObj The enum (or enum-like) object to traverse.
 * @param callback A function that will be called with each key and its corresponding value of enum entry.
 * @param lastEntry value to be processed last.
 */
function traverseEnum(enumObj, callback, lastEntry) {
    const entries = [
        ...new Set(Object.keys(enumObj).filter((key) => isNaN(Number(key)) && enumObj[key] !== lastEntry))
    ];
    const lastKey = Object.keys(enumObj).find((key) => enumObj[key] === lastEntry);
    entries.push(lastKey);
...
function fillTableInConfig(importParameters, manifestSettings, configPart, sectionId, factory, tableId) {
    ...
    if (manifestSection) {
        ...
        const validTableType = tableSettings['type'];
        traverseEnum(v4_1.TableTypeV4, (key, tableType) => {
            const tableDefinitionName = getTableDefinitionNameByType(tableType);
            // Data needs to be transferred
            configPart[sectionId] = configPart[sectionId] ?? {};
            configPart[sectionId].table = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, tableDefinitionName, configPart[sectionId].table);
            const sectionDefinition = `${tableDefinitionName}<${tableId ? getDefinitionId(tableId) : sectionId.replace('#', '::')}>`;
            (0, utils_1.importSettingsOfObject)(configPart[sectionId].table, importParameters.manifest, importParameters.jsonSchema.definitions[sectionDefinition], importParameters.routingId, [], manifestSectionId, undefined, importParameters.jsonSchema.definitions);
 ...
 * @param sectionId - ID of the current section
 * @param factory - factory, for generating the access to reflect-metadata
 * @param includeActions - Whether to include action configurations in the form(default is 'true')
 */
function fillFormInConfig(importParameters, manifestSettings, configPart, sectionId, factory, includeActions = true) {
    const manifestSectionId = getManifestId(sectionId);
    const manifestSection = manifestSettings.controlConfiguration?.[manifestSectionId];
    if (manifestSection) {
        configPart[sectionId] = {};
        const initialConfig = includeActions ? { actions: {} } : {};
        configPart[sectionId].form = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, ux_specification_types_1.DefinitionName.ObjectPageForm, initialConfig);
        if (includeActions) {
            fillFormActionsInConfig(importParameters, manifestSection, sectionId, configPart, factory, manifestSectionId);
        }
        // fill custom fields
        fillFieldInConfig(importParameters, manifestSection, sectionId, configPart, factory, manifestSectionId);
    }
}
/**
...
function fillSectionsInConfig(importParameters, sectionProperties, manifestSettings, configPart, factory, parentId) {
    for (const sectionId in sectionProperties) {
        ...
        if ((sectionId.includes("com.sap.vocabularies.UI.v1.PresentationVariant" /* UIAnnotationTerms.PresentationVariant */) ||
            sectionId.includes("com.sap.vocabularies.UI.v1.SelectionPresentationVariant" /* UIAnnotationTerms.SelectionPresentationVariant */)) &&
            ...
            (schemaSection.$ref?.includes(`@${"com.sap.vocabularies.UI.v1.FieldGroup" /* UIAnnotationTerms.FieldGroup */}::`) ||
                schemaSection.$ref?.includes(`@${"com.sap.vocabularies.UI.v1.Identification" /* UIAnnotationTerms.Identification */}`))) {
            // Form section
            fillFormInConfig(importParameters, manifestSettings, configPart, sectionId, factory, !schemaSection.$ref.includes(`@${"com.sap.vocabularies.UI.v1.Identification" /* UIAnnotationTerms.Identification */}`));
        }
 ...
 */
function fillHeaderFacets(appSchema, objectPageConfig, factory, manifest, manifestSettings, routingId) {
    ...
    if (headerSections && headerSections.properties) {
        ...
        for (const sectionId in headerSections.properties) {
            ...
            if (!isCustomHeaderSection) {
                ...
                if (manifestSection) {
                    fillFieldInConfig({
                        jsonSchema: appSchema,
                        manifest,
                        routingId
                    }, manifestSection, sectionId, objectPageConfig.header.sections, factory, manifestSectionId, ux_specification_types_1.DefinitionName.ObjectPageHeaderSectionForm);
                }
            }
        }
    }
...
function fillObjectPageConfig(importParameters, v4Page, factory) {
    ...
    if (manifestSettings) {
        ...
        // Fill header facets
        fillHeaderFacets(importParameters.jsonSchema, objectPageConfig, factory, importParameters.manifest, manifestSettings, importParameters.routingId);
