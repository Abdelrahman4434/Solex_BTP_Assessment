...
/**
 * Determines the target annotation for a header section field group.
 *
 * @param ids - List of IDs of parent elements.
 * @returns An object containing encoded target annotation, and decoded target annotation.
 */
function determineTargetAnnotationOfHeaderSectionFieldsGroups(ids) {
    const sectionId = (0, utils_2.extractLastIdPart)(ids[2] ?? '');
    return { targetAnnotationEncoded: sectionId.replace(/\//g, '::'), targetAnnotation: sectionId };
}
/**
...
function determineTargetAnnotation(ids, transferParameters, configObject) {
    ...
    else if (ids[1] === ux_specification_types_1.PropertyName.sections && ids[4] === 'fields') {
        ({ targetAnnotationEncoded, targetAnnotation } = determineTargetAnnotationOfHeaderSectionFieldsGroups(ids));
    }
...
function transferSortedListEntry(parameters, parentIds, stableIdParts, preprocessed = false) {
    ...
    const nextDefinitions = (0, common_1.getNextTargetDefinitions)(parameters.transferParameters.appSchema, parameters.transferParameters.title, currentConfigObject, parameters.propertyDefinition, parameters.key, parameters.transferParameters.factory, parameters.transferParameters.pageType) ?? [undefined];
