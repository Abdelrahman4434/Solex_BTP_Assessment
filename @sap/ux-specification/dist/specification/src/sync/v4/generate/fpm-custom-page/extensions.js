 ...
 * Handles both direct class references and interface references (which are resolved to their implementations).
 *
 ...
 * @returns {boolean} True if the schema supports custom extensions with anchor/placement properties.
 */
 ...
 * @returns {AnnotationPrefix | undefined} The annotation prefix object or undefined if no prefix is found.
 */
...
function getExtensionData(node, schema, property, aggregationName) {
    ...
    const placement = (0, utils_3.getAttribute)(xmlExtension, PLACEMENT_PROPERTIES.placement) || ux_specification_types_2.v4.Placement.After;
...
function updatePropertyIndices(node, schema, aggregationName) {
    ...
    (0, utils_4.addExtensionToArray)(extensions, sortedExtensionsArray, '', ux_specification_types_1.TemplatePropertyName.Position);
 ...
 * @returns {string} Corrected anchor enum key.
 */
...
function applyEnumsForAnchors(node, schema, appSchema, aggregationName) {
    ...
    for (const name in schema.properties) {
        ...
        // Remove annotation prefix from anchor enum entries - it is difference between manifest and macros anchoring
