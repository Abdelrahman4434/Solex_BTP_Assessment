...
function addReferredTypesFromType(type, globalTypes) {
    if (globalTypes.referredTypeNames.has(type)) {
...
/**
 * Helper function that transforms the given AccessorSpec into a JSONPath string.
 *
 * @param path - the path to the property
 * @param property - name of the property
 * @returns a JSONPath string identifying the access to the property
 */
function getJSONPathForProperty(path, property) {
    const pathIsArray = Array.isArray(path);
    const pathAsArray = pathIsArray ? path : path.path;
    if (pathIsArray || path.addTargetPropertyToPath) {
        pathAsArray.push(property);
    }
    let jsonPath = '$';
    pathAsArray.forEach((segment) => {
        ...
        jsonPath = (0, common_1.appendJsonPathSegment)(jsonPath, currentSegment, previousSegment);
    });
    return jsonPath;
}
/**
    ...
    for (const property in properties) {
        ...
        if (processingRule.element) {
            ...
            if (processingRule.controlId) {
                ...
                processingRule.element[ux_specification_types_1.SchemaTag.manifestPath] = getJSONPathForProperty(processingRule.manifestPath, property);
            }
