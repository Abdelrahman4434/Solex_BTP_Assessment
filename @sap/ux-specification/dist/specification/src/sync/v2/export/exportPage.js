...
// Temporary solution:
// pageAccess.ts will reroute its function exportListReportPage to the function with the same name in this file.
// Hence, exportListReportPageV2 is actually the function exportListReportPage from this file.
...
/**
 * Creates and returns a new export results object initialized with default values.
 *
 * @param fragmentFiles - Optional array of file data to include in the export results.
 * @returns An object containing default export results, including flex changes, manifest, and fragments.
 */
...
const exportApplicationV2 = (application, manifest, jsonSchema) => {
    ...
    for (const key in jsonSchema.definitions.AppSettings['properties']) {
        ...
        if (syncRule && syncRule.manifest) {
            ...
            const exportKey = syncRule.manifest.key ?? key;
            let manifestSection = (0, manifest_1.getManifestSectionByPathV2)(exportResults.manifest, path);
            if (application.settings && application.settings[key] !== undefined) {
                manifestSection = manifestSection ?? (0, manifest_1.ensureManifestSectionByPathV2)(exportResults.manifest, path);
                const prevValue = manifestSection[exportKey];
                manifestSection[exportKey] = application.settings[key];
                // If the previous value was an object with entries and now it is an empty object, call deleteEmptyStructure
                if (typeof prevValue === 'object' &&
                    Object.keys(prevValue).length > 0 &&
                    typeof manifestSection[exportKey] === 'object' &&
                    Object.keys(manifestSection[exportKey]).length === 0) {
                    delete manifestSection[exportKey];
                    (0, common_1.deleteEmptyStructure)(exportResults.manifest, path, false);
                }
            }
            else if (manifestSection && exportKey in manifestSection) {
                delete manifestSection[exportKey];
                (0, common_1.deleteEmptyStructure)(exportResults.manifest, path, false);
            }
        }
...
function exportPageV2(exportParametersV2, ui5Version) {
    if (exportParametersV2[ux_specification_types_1.SchemaType.ListReport]) {
        return (0, pageAccess_1.exportListReportPage)(exportParametersV2[ux_specification_types_1.SchemaType.ListReport], ui5Version);
    }
...
function exportConfigEntityByPathV2(exportParametersV2, ui5Version, deletionRequest = false) {
    if (exportParametersV2[ux_specification_types_1.SchemaType.ListReport]) {
        return (0, pageAccess_1.exportListReportPage)(exportParametersV2[ux_specification_types_1.SchemaType.ListReport], ui5Version, deletionRequest);
    }
    else if (exportParametersV2[ux_specification_types_1.SchemaType.ObjectPage]) {
