...
function getFileList(appSchemas, configs) {
    let files = [];
    for (const schemaName of Object.keys(appSchemas).sort((a, b) => a.localeCompare(b))) {
        let dataSourceUri;
...
async function importProjectSchema(importProjectParameters) {
    // Initialize i18next
    (0, i18n_1.initI18n)();
    const { manifest, annotations, flex, logger, fragments, views, featureToggles } = importProjectParameters;
